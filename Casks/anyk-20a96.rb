cask "anyk-20a96" do
  ...
  url "https://nav.gov.hu/pfile/programFile?path=/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/20A96/NAV_20A96"
  name "NAV 20A96 Template"
...
pénzügyi intézmények részére "
  homepage "https://nav.gov.hu/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/20A96"
  ...
  preflight do
    ...
    system_command "/usr/bin/unzip",
                   args: ["-o", "-q", "#{staged_path}/NAV_20A96.jar", "application/*", "-d", staged_path.to_s],
                   print_stderr: false
  ...
  uninstall_preflight do
    ...
    if File.exist?(config_file)
      anyk_dir = File.read(config_file).strip
      Dir.glob("#{anyk_dir}/nyomtatvanyok/*20A96*.tem.enyk").each do |f|
        FileUtils.rm_f(f)
      end
    end
  end
  ...
  caveats <<~EOS
    NAV 20A96 template has been installed automatically.
