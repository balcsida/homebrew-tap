cask "anyk-2448" do
  ...
  url "https://nav.gov.hu/pfile/programFile?path=/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/2448/nav_2448"
  name "NAV 2448 Template"
  ...
  homepage "https://nav.gov.hu/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/2448"
  ...
  preflight do
    ...
    system_command "/usr/bin/unzip",
                   args: ["-o", "-q", "#{staged_path}/nav_2448.jar", "application/*", "-d", staged_path.to_s],
                   print_stderr: false
  ...
  uninstall_preflight do
    ...
    if File.exist?(config_file)
      anyk_dir = File.read(config_file).strip
      Dir.glob("#{anyk_dir}/nyomtatvanyok/*2448*.tem.enyk").each do |f|
        FileUtils.rm_f(f)
      end
    end
  end
  ...
  caveats <<~EOS
    NAV 2448 template has been installed automatically.
