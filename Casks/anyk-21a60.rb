cask "anyk-21a60" do
  ...
  url "https://nav.gov.hu/pfile/programFile?path=/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/21A60/NAV_21A60"
  name "NAV 21A60 Template"
...
termékbeszerzések és szolgáltatás igénybevételekről"
  homepage "https://nav.gov.hu/nyomtatvanyok/letoltesek/nyomtatvanykitolto_programok/nyomtatvanykitolto_programok_nav/21A60"
  ...
  preflight do
    ...
    system_command "/usr/bin/unzip",
                   args: ["-o", "-q", "#{staged_path}/NAV_21A60.jar", "application/*", "-d", staged_path.to_s],
                   print_stderr: false
  ...
  uninstall_preflight do
    ...
    if File.exist?(config_file)
      anyk_dir = File.read(config_file).strip
      Dir.glob("#{anyk_dir}/nyomtatvanyok/*21A60*.tem.enyk").each do |f|
        FileUtils.rm_f(f)
      end
    end
  end
  ...
  caveats <<~EOS
    NAV 21A60 template has been installed automatically.
